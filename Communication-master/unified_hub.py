        ...
        async for msg in ws:
            if msg.type == web.WSMsgType.TEXT:
                ...
                # --- B. Key Exchange & Routing ---
                    ...
                    # Store the keys for this client so late joiners can get them
                    ...
                    if target == "drone": # Broadcast the keys to everyone else
                        ...
                        for cid, c in clients.items():
                            if cid != client_id and "keys" in c:
...
async def main():
    ...
    try:
        ...
        info = ServiceInfo(
            "_http._tcp.local.",
            f"Drone Unified Mesh {suffix}._http._tcp.local.",
            addresses=[socket.inet_aton(local_ip)],
            port=args.port,
            properties=desc,
            server=f"drone-mesh-{suffix}.local.",
        )
        zeroconf.register_service(info)
        logging.info(f"🛰️ Broadcasting as 'http://drone-mesh-{suffix}.local:{args.port}'")
    except Exception:
        # mDNS frequently fails on some Windows interfaces, but it's optional
        pass
